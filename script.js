...
const representativeBox = document.getElementById("representativeBox");
const representativeName = document.getElementById("representativeName");

participantCount.addEventListener("change", () => {
  const value = participantCount.value;

  if (value !== "1" && value !== "") {
    representativeBox.classList.remove("hidden");
    representativeName.required = true;
  } else {
    representativeBox.classList.add("hidden");
